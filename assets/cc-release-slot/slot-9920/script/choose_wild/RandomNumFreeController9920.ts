...
import { ItemRandomNumFree9920 } from './ItemRandomNumFree9920';
...
@ccclass('RandomNumFreeController9920')
export class RandomNumFreeController9920 extends Component {
    @property(ItemRandomNumFree9920)
    listItemRandom: ItemRandomNumFree9920[] = [];

    _spiningTime: number = 1.5;
    ...
    private _updateData() {
        for (let i = 0; i < this.listItemRandom.length; i++) {
            ...
            item.setNumFree(multi[0] + + SlotConfig9920.extraFreeGame);
        }
    }
    ...
    setData4MainItem() {
        for (let i = 0; i < this.listItemRandom.length; i++) {
            ...
            if (pos.y > this._h_item && pos.y < this._h_item * 2) {
                item.setNumFree(SlotConfig9920.currentOptionFree[0]);
            }
        }
    }
}