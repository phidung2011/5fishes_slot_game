...
export default class ExtTimelineTaskMgr extends cc.Component {
    private _pool: ExtTimelineTask[] = [];
    private _intervalId: number = -1;
    private _dt: number = 0.025;
    private _isRunning = true;
    private _lastestTimeStamp: number = 0;

    private static _instance: ExtTimelineTaskMgr = null!;
    public static get instance(): ExtTimelineTaskMgr {
        if (ExtTimelineTaskMgr._instance == null) {
            ExtTimelineTaskMgr._instance = new ExtTimelineTaskMgr();
        }
        return ExtTimelineTaskMgr._instance;
    }
    ...
    scheduleTask(key: string, updateFunc: Function, caller: any | null = null, args: any[] | null = null, overide: boolean = true, delay: number = 0) {
        ...
        let task = new ExtTimelineTask(key, updateFunc, caller, args, delay);
