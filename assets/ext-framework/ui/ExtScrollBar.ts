...
export enum ExtScrollBarDirection {
    VERTICAL = 1,
    HORIZONTAL = 2
}

@ccclass
export default class ExtScrollBar extends cc.Component {
    ...
    @property({
        type: cc.Enum(ExtScrollBarDirection)
    })
    direction: ExtScrollBarDirection = ExtScrollBarDirection.VERTICAL;

    onLoad() {
        if (this.scrollView && (this.direction & this.scrollView.direction) != 0 && this.bar && (this.direction == ExtScrollBarDirection.HORIZONTAL || this.direction == ExtScrollBarDirection.VERTICAL)) {
    ...
    updateBackground() {
        if (this.scrollView) {
            ...
            if (this.direction == ExtScrollBarDirection.VERTICAL) {
            ...
            } else if (this.direction == ExtScrollBarDirection.HORIZONTAL) {
