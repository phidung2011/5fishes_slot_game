...
import ExtScrollContent from "./ExtScrollContent";
import ExtControlEvent from "./ExtControlEvent";

const { ccclass, property, type } = cc._decorator;

export enum ExtScrollDirection {
    VERTICAL = 1,
    HORIZONTAL = 2,
    BOTH = 3
}

export interface ExtScrollViewDelegate {
    scrollViewDidScroll?(scrollView: ExtScrollView): void;
    // called on start of dragging (may require some time and or distance to move)
    scrollViewWillBeginDragging?(scrollView: ExtScrollView): void;
    // called on finger up if the user dragged. velocity is in points/millisecond.
    scrollViewWillEndDragging?(scrollView: ExtScrollView, velocity: cc.Vec2): void;
    // called on finger up if the user dragged. decelerate is true if it will continue moving afterwards
    scrollViewDidEndDragging?(scrollView: ExtScrollView, decelerate: boolean): void;

    scrollViewChildrenShouldDrag?(scrollView: ExtScrollView, event: cc.EventTouch): boolean;
}

export interface ExtScrollViewDisabledRefObject {
    scrollViewShouldScroll?(scrollView: ExtScrollView): boolean;
}

@ccclass
export default class ExtScrollView extends ExtViewGroup {
    ...
    private _disabledRefObjects: ExtScrollViewDisabledRefObject[] = [];

    public retainDisabledRef(obj: ExtScrollViewDisabledRefObject) {
        if (this._disabledRefObjects.indexOf(obj) < 0) {
            this._disabledRefObjects.push(obj)
        }
    }

    public releaseDisabledRef(obj: ExtScrollViewDisabledRefObject) {
    ...
    public get delegate(): ExtScrollViewDelegate {
        return this._delegate;
    }

    public set delegate(value: ExtScrollViewDelegate) {
        this._delegate = value;
    }
    protected _delegate: ExtScrollViewDelegate = null!;

    @property({
        type: cc.Enum(ExtScrollDirection)
    })
    direction: ExtScrollDirection = ExtScrollDirection.BOTH;
    ...
    // @property(ExtScrollBar)
    // scrollBar: ExtScrollBar = null!;
    ...
    onLoad() {
        if (this.content) {
            let contentComp = this.content.getComponent(ExtScrollContent) as ExtScrollContent;
            if (!contentComp) {
                contentComp = this.content.addComponent(ExtScrollContent);
            }
    ...
    onDisable() {
        ...
        // }
        // cc.Canvas.instance.node.off(ExtControlEvent.TouchRemoved, this.onTouchRemoved, this);
    }
    ...
    protected update(dt: number) {
        if (this._autoZoomScale) {
            ...
            percentage = ExtScrollView.quintEaseOut(percentage);
    ...
    protected _registerEvent() {
        ...
        cc.find('Canvas')?.on(ExtControlEvent.CanvasCancel, this._onShowGame, this);
    }
    ...
    protected _unregisterEvent() {
        ...
        cc.find('Canvas')?.off(ExtControlEvent.CanvasCancel, this._onShowGame, this);
    }
    ...
    setContentPosition(position: cc.Vec2, t: boolean = true) {
        ...
        if (this.content.position.x != position.x || this.content.position.y != position.y) {
            ...
            if ((this.direction & ExtScrollDirection.HORIZONTAL) != 0) {
                this.content.setPosition(position.x, this.content.position.y);
            }

            if ((this.direction & ExtScrollDirection.VERTICAL) != 0) {
