...
import ExtScrollView, { ExtScrollViewDelegate, ExtScrollDirection } from "./ExtScrollView";
import ExtTableCell from "./ExtTableCell";

export enum ExtTableViewFillOrder {
    LEFT_TO_RIGHT__TOP_TO_BOTTOM = 0,
    TOP_TO_BOTTOM__LEFT_TO_RIGHT = 1
}

export enum ExtTableViewInteractionMode {
    NONE,
    CLICK,
    SINGLE_SELECTION,
    MULTIPLE_SELECTION
...
export interface ExtTableViewDelegate extends ExtScrollViewDelegate {
    tableCellClicked?(tableView: ExtTableView, cell: cc.Node, idx: number): void;

    tableCellDidSelected?(tableView: ExtTableView, idx: number): void;

    tableCellWillDeselected?(tableView: ExtTableView, idx: number): void;

    tableCellWillRecycle?(tableView: ExtTableView, cell: cc.Node, idx: number): void;

    tableCellShouldHighlight?(tableView: ExtTableView, idx: number): boolean;

    tableCellDidUnhighlight?(tableView: ExtTableView, idx: number): void;

    tableCellDidHighlight?(tableView: ExtTableView, idx: number): void;

    tableCellShouldSelect?(tableView: ExtTableView, idx: number): boolean;

    tableClickOnSelectedCell?(tableView: ExtTableView, idx: number): void;
}

export interface ExtTableViewDataSource {
    numberOfCellsInTableView(tableView: ExtTableView): number;
    tableCellAtIndex(tableView: ExtTableView, idx: number): cc.Node;
    tableCellSize?(tableView: ExtTableView): cc.Size;
    tableCellAnchor?(tableView: ExtTableView): cc.Vec2;
}

@ccclass
export default class ExtTableView extends ExtScrollView {
    @property({
        type: cc.Enum(ExtTableViewFillOrder)
    })
    private fillOrder: ExtTableViewFillOrder = ExtTableViewFillOrder.LEFT_TO_RIGHT__TOP_TO_BOTTOM;

    @property({
        type: cc.Enum(ExtTableViewInteractionMode)
    })
    private interactionMode: ExtTableViewInteractionMode = ExtTableViewInteractionMode.NONE;
    ...
    public get delegate(): ExtTableViewDelegate {
        return this._tableViewDelegate;
    }

    public set delegate(value: ExtTableViewDelegate) {
        this._tableViewDelegate = value;
    }

    private _tableViewDelegate: ExtTableViewDelegate = null!;

    public get dataSource(): ExtTableViewDataSource {
        return this._dataSource;
    }

    public set dataSource(value: ExtTableViewDataSource) {
        this._dataSource = value;
    }
    private _dataSource: ExtTableViewDataSource = null!;
    ...
    onLoad() {
        ...
        let cache = ExtTableView._cellPoolCache[cacheKey];
        if (!cache) {
            cache = ExtTableView._cellPoolCache[cacheKey] = new cc.NodePool(cacheKey + "<TableCellPool>");
        }
        ...
        if (this.direction == ExtScrollDirection.BOTH) {
            this.direction = ExtScrollDirection.VERTICAL;
        }
    }

    onEnable() {
        super.onEnable();
        if (this.interactionMode != ExtTableViewInteractionMode.NONE) {
    ...
    protected _onMouseWheel(event: cc.EventMouse, captureListeners?: Node[]) {
        ...
        if (this.direction == ExtScrollDirection.VERTICAL) {
            deltaMove.set(0, scrollY * wheelPrecision);
        } else if (this.direction == ExtScrollDirection.HORIZONTAL) {
    ...
    private _setSelectedForCell(cell: cc.Node, selected: boolean, forceSelected: boolean = false) {
        let cellComp = cell.getComponent(ExtTableCell) as ExtTableCell;
    ...
    public selectCellAtIndex(index: number) {
        ...
        if (this.interactionMode == ExtTableViewInteractionMode.NONE ||
            this.interactionMode == ExtTableViewInteractionMode.CLICK ||
        ...
        if (!this._tableViewDelegate || !this._tableViewDelegate.tableCellShouldSelect || this._tableViewDelegate.tableCellShouldSelect(this, index)) {
            ...
            if (this.interactionMode == ExtTableViewInteractionMode.SINGLE_SELECTION) {
    ...
    public deselectCellAtIndex(index: number) {
        if (this.interactionMode == ExtTableViewInteractionMode.NONE ||
            this.interactionMode == ExtTableViewInteractionMode.CLICK) return;
    ...
    private _onContentClick(touch: cc.Touch) {
        ...
        if (this.interactionMode != ExtTableViewInteractionMode.NONE) {
            ...
            let cell = this.cellAtIndex(index);
            if (cell) {
                if (this.interactionMode == ExtTableViewInteractionMode.CLICK) {
                ...
                } else if (this.interactionMode == ExtTableViewInteractionMode.SINGLE_SELECTION) {
                ...
                } else if (this.interactionMode == ExtTableViewInteractionMode.MULTIPLE_SELECTION) {
                    if (this._selectedIndices.length == 0) {
                        if (!this._tableViewDelegate ||
                            !this._tableViewDelegate.tableCellShouldSelect ||
                            this._tableViewDelegate.tableCellShouldSelect(this, index)) {
    ...
    protected scrollViewDidScroll(scrollView: ExtScrollView) {
    ...
    updateCellAtIndex(idx: number) {
        ...
        if (idx >= this._startIdx && idx <= this._endIdx) {
            ...
            switch (this.interactionMode) {
                case ExtTableViewInteractionMode.SINGLE_SELECTION:
                case ExtTableViewInteractionMode.MULTIPLE_SELECTION:
    ...
    private _indexFromOffset(offset: cc.Vec2) {
        ...
        if (this.direction == ExtScrollDirection.VERTICAL) {
            ...
            if (this.fillOrder == ExtTableViewFillOrder.LEFT_TO_RIGHT__TOP_TO_BOTTOM) {
                if (idxX >= this._cols) {
                    return -1;
                }
    ...
    private _updateGridData(): void {
        ...
        if (cellsCount > 0) {
            switch (this.direction) {
                case ExtScrollDirection.HORIZONTAL:
                    {
                    ...
                    break;
                case ExtScrollDirection.VERTICAL:
                    {
