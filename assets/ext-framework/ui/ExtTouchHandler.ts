...
@ccclass
export default class ExtTouchHandler extends cc.Component {
    ...
    protected _onTouchBegan(event: cc.EventTouch): void {
        ...
        node.emit(ExtControlEvent.TouchDown, touch);
        this._pressed = true;
    ...
    protected _onTouchMoved(event: cc.EventTouch) {
        ...
        if (this._lastHits[touch.__instanceId] != hit) {
            if (hit) {
                this.node.emit(ExtControlEvent.TouchDragEnter, touch);
                this.node.emit(ExtControlEvent.TouchDragInside, touch);
            } else {
                this.node.emit(ExtControlEvent.TouchDragExit, touch);
                this.node.emit(ExtControlEvent.TouchDragOutside, touch);
            }
    ...
    protected _onTouchEnded(event: cc.EventTouch) {
        ...
        if (this._pressed) {
            ...
            if (!this._longPressed) {
                ...
                this.node.emit(ExtControlEvent.ClickByTouchHandler, touch);
            } else {
                this._longPressed = false;
            }
        }

        this.node.emit(ExtControlEvent.TouchUpInside, touch);
    ...
    protected _onTouchCancelled(event: cc.EventTouch) {
        ...
        this._longPressed = false;
        let touch = event.touch as any;
        this.node.emit(ExtControlEvent.TouchUpOutside, touch);
        delete this._lastHits[touch.__instanceId];
        this.unscheduleAllCallbacks();
    }
    ...
    private _longClickExec(dt: number) {
        if (this.longClickEnabled && this._pressed && !this._longPressed) {
            this.node.emit(ExtControlEvent.LongClick, this);
